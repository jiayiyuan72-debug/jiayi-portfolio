...
/** 左侧组件库：点击即添加，也支持拖拽 */
export default function ComponentPalette({ onDragStart, onPointerStart, onAddClick }: Props) {
  const render = (t: CanvasType) => (
    <div
      ...
      title={`点击或拖入${CANVAS_TYPE_LABELS[t].label}`}
    >
  ...
  return (
    <div className="w-44 bg-white border-r border-[#e8e4de] p-3 overflow-y-auto flex-shrink-0">
      <p className="text-xs text-[#8b8b8b] mb-2">组件库（点击或拖入画板）</p>
